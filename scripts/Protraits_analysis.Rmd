...
# # 3 different paths for the 3 different computer used for this analysis. 
...
protraits <- read_csv("data/modified/protraits_20200225") %>% 
  select(-c(protname, zscore, cscore, googlehits))
...
# lambda - L2 regularization term on weights (analogous to Ridge regression) - add this to parameters?

nrow(hyper_grid)

# grid search 

# Put this loop in a loop to cycle through different seeds, tune parameters based on averages across seeds? idk, I'm tired.
for(i in 1:nrow(hyper_grid)) {
  ...
  xgb.tune <- xgb.cv(
    ...
    verbose = 0,                   # silent
    early_stopping_rounds = 25,     # stop if no improvement for 25 consecutive trees
...
# added columns to calculate difference between training and test metrics to evaluate over-fitting

#c hange na.action back to default
...
#this arrange thing doesn't seem to be working
...
# Use this to fine tune parameters, save outputs from different tests. 
...
## FROM OLD PROTRAITS_ANALYSIS_2.RMD 
