...
# What is the proportion of positives in the training dataset?
10/149 # 0.06711409

# What is the proportion of positives in the full dataset?
13/228 # 0.05701754
...
filter(pred.df, true.zoostat == 1) %>%
  summarise(min(pred.zoostat)) # 0.00947009

#AUC (to explore that threshold)
roc.test <- pROC::roc(response = pred.df$true.zoostat, predictor = pred.df$pred.zoostat)
pROC::auc(roc.test) # 0.7043
plot(roc.test)
roc.test$thresholds[which.max(roc.test$sensitivities+ roc.test$specificities)] #0.02291124

# Set cutoff threshold
pred.class <- ifelse(pred >= 0.02323, 1, 0) %>% as.factor()
